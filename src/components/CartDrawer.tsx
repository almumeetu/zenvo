...
export const CartDrawer: React.FC<CartDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-zenov-bg/75 backdrop-blur-sm">
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-md bg-zenov-card border-l border-zenov-border text-zenov-text flex flex-col shadow-xl">
          {/* Header */}
          <div className="p-4 sm:p-5 border-b border-zenov-border flex items-center justify-between bg-zenov-surface/60">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-lg bg-zenov-primary-soft border border-zenov-primary-border flex items-center justify-center">
                <ShoppingBag className="w-[18px] h-[18px] text-zenov-primary" />
              </div>
              <div>
                <h2 className="text-base font-bold text-zenov-text">Your Cart</h2>
            ...
            <button
              onClick={onClose}
              className="p-2 rounded-lg bg-zenov-surface border border-zenov-border hover:border-zenov-border-hover text-zenov-text-secondary hover:text-zenov-text transition-all"
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3">
            {cartItems.length === 0 ? (
              <div className="text-center py-20 space-y-4">
                <div className="w-20 h-20 bg-zenov-surface border border-zenov-border rounded-2xl flex items-center justify-center mx-auto text-zenov-text-muted">
                  <ShoppingBag className="w-9 h-9" />
                </div>
                <div>
                  <p className="text-sm font-semibold text-zenov-text">Your cart is empty</p>
                  <p className="text-xs text-zenov-text-muted mt-1">
          ...
          {/* Checkout */}
          {cartItems.length > 0 && (
            <div className="p-4 sm:p-5 border-t border-zenov-border bg-zenov-surface/60 space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-zenov-border/70">
                <div>
                  <p className="text-xs text-zenov-text-muted">Total Amount</p>
                  <p className="text-[11px] text-zenov-text-secondary">
                    {cartItems.reduce((a, i) => a + i.quantity, 0)} items
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-2xl font-black text-zenov-text font-mono tracking-tight">
                    {formatCurrency(totalUSD, selectedCurrency)}
                  </p>
                  <p className="text-[10px] text-zenov-success font-medium">Instant Delivery</p>
                </div>
              </div>

              <button
                onClick={onCheckout}
                className="w-full py-3.5 rounded-lg bg-zenov-accent hover:bg-zenov-accent-hover text-zenov-bg font-bold text-sm uppercase tracking-wide shadow-accent hover:shadow-lg transition-all duration-200 inline-flex items-center justify-center gap-2 active:scale-[0.99] will-change-transform"
              >
