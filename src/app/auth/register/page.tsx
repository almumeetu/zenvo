...
function RegisterContent() {
  ...
  return (
    <div className="min-h-[calc(100vh-200px)] flex items-center justify-center py-10 px-4 sm:px-6">
      <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 rounded-3xl overflow-hidden border border-zenov-border bg-zenov-card shadow-2xl">
        {/* LEFT: form */}
          ...
          {successMsg ? (
            ...
            <form onSubmit={onSubmit} className="space-y-3.5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                <div>
                  <label className="text-[11px] font-bold uppercase tracking-wider text-zenov-text-muted block mb-1.5">Full Name *</label>
                ...
                <div>
                  <label className="text-[11px] font-bold uppercase tracking-wider text-zenov-text-muted block mb-1.5">Phone (optional)</label>
                  <div className="relative">
                    <Smartphone className="w-4 h-4 text-zenov-text-muted absolute left-3.5 top-1/2 -translate-y-1/2" />
                    <input
                      type="tel"
                      value={form.phone}
              ...
              <div>
                <label className="text-[11px] font-bold uppercase tracking-wider text-zenov-text-muted block mb-1.5">Email *</label>
                <div className="relative">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                <div>
                  ...
                  <div className="relative">
                    ...
                    />
                    <button type="button" onClick={() => setShowPw((s) => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-zenov-text-muted hover:text-zenov-text" aria-label="Toggle">
                      {showPw ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                    </button>
                  </div>
                </div>
                <div>
                  <label className="text-[11px] font-bold uppercase tracking-wider text-zenov-text-muted block mb-1.5">Confirm password *</label>
                  <div className="relative">
                    <Lock className="w-4 h-4 text-zenov-text-muted absolute left-3.5 top-1/2 -translate-y-1/2" />
                    <input
                      ...
                      className="w-full pl-10 pr-10 py-3 rounded-xl bg-zenov-surface border border-zenov-border focus:border-zenov-primary-border focus:ring-2 focus:ring-zenov-primary-border/40 outline-none transition-all text-sm"
                    />
                    <button type="button" onClick={() => setShowPw2((s) => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-zenov-text-muted hover:text-zenov-text" aria-label="Toggle">
              ...
              <label className="inline-flex items-start gap-2.5 text-xs cursor-pointer select-none pt-1">
                <input
                  ...
                  className="accent-zenov-primary w-4 h-4 mt-0.5 rounded"
                />
                <span className="text-zenov-text-secondary leading-relaxed">
                  I agree to the <a href="#" className="text-zenov-primary font-semibold hover:underline">Terms of Service</a> and understand my data is stored securely per the <a href="#" className="text-zenov-primary font-semibold hover:underline">Privacy Policy</a>.
                </span>
              </label>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3.5 rounded-xl bg-gradient-to-r from-zenov-accent via-orange-500 to-zenov-accent-hover text-zenov-bg text-sm font-black uppercase tracking-wider shadow-md hover:shadow-lg disabled:opacity-50 active:scale-[0.98] transition-all inline-flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>Creating Account <div className="w-4 h-4 border-2 border-zenov-bg/30 border-t-zenov-bg rounded-full animate-spin" /></>
                ) : (
                  <>
        ...
        {/* RIGHT: perks */}
        <div className="hidden lg:flex flex-col justify-between p-10 bg-gradient-to-br from-zenov-accent-soft via-orange-950/30 to-zenov-primary-soft relative overflow-hidden order-1 lg:order-2">
          <div className="absolute -top-32 -right-32 w-80 h-80 rounded-full bg-zenov-accent/30 blur-3xl" />
          <div className="absolute -bottom-32 -left-32 w-80 h-80 rounded-full bg-zenov-primary/20 blur-3xl" />

          <div className="relative flex items-center justify-end">
            <div className="px-3 py-1.5 rounded-lg bg-zenov-bg/40 border border-zenov-border/50 text-xs font-bold uppercase tracking-wider text-zenov-text inline-flex items-center gap-1.5">
              <Crown className="w-3.5 h-3.5 text-zenov-accent" /> Welcome Bonus
            </div>
          </div>

          <div className="relative space-y-7">
            <div>
              <h2 className="text-4xl font-black text-zenov-text leading-[1.05] tracking-tight">
                Join 1M+<br/>Gamers Worldwide.
              </h2>
              <p className="text-zenov-text-secondary text-sm mt-3 max-w-sm">
                Sign up today and receive a welcome bonus + exclusive first-order promo code instantly in your inbox.
              </p>
            </div>

            <div className="p-4 rounded-2xl bg-zenov-bg/40 border border-zenov-border/50 backdrop-blur inline-flex items-center gap-4">
              <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-zenov-primary to-zenov-accent text-zenov-bg flex items-center justify-center shadow-md">
                <Gift className="w-7 h-7" />
              </div>
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-zenov-accent">Sign-up Gift</p>
                <p className="text-2xl font-black text-zenov-text font-mono">10% OFF</p>
                <p className="text-[10px] text-zenov-text-muted font-semibold">Code: WELCOME5 (auto applied)</p>
              </div>
            </div>
            ...
            <div className="space-y-2.5">
              {[
                ...
                <div key={t} className="flex items-center gap-3 p-3 rounded-xl bg-zenov-bg/30 border border-zenov-border/50 backdrop-blur">
                  <div className="w-9 h-9 rounded-lg bg-zenov-accent-soft text-zenov-accent flex items-center justify-center shrink-0">
                    <I className="w-4 h-4" />
                  </div>
                  <div>
