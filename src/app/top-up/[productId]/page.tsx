...
export default function TopUpPage() {
  ...
  const verifyPlayer = () => {
    ...
    // Mock verification
    setTimeout(() => {
      const names = ['ShadowHunter', 'PhoenixRider', 'NoobMaster69', 'EpicGamer', 'ZenvoKing'];
      setVerifiedPlayer({
  ...
  return (
    <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
      ...
      <div className="flex items-center gap-2 text-xs text-zenvo-text-muted mb-6 flex-wrap">
        ...
        <ChevronRight className="w-3 h-3" />
        <Link href={`/shop?cat=${product.category}`} className="hover:text-zenvo-primary transition-colors capitalize">
          {product.category.replace('-', ' ')}
        </Link>
        <ChevronRight className="w-3 h-3" />
        <span className="text-zenvo-text font-semibold truncate max-w-[240px]">{product.title}</span>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 lg:gap-8">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 p-5 sm:p-7 rounded-3xl bg-zenvo-card border border-zenvo-border">
            <div className="relative rounded-2xl overflow-hidden bg-zenvo-surface aspect-square border border-zenvo-border">
              <img
                src={product.image}
                alt={product.title}
              ...
              <div className="absolute top-3 left-3 flex flex-col gap-1.5">
                {product.deliveryType === 'Instant' && (
                  <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-zenvo-success text-zenvo-bg text-[10px] font-bold uppercase tracking-wider shadow-sm">
                    <Zap className="w-2.5 h-2.5 fill-zenvo-bg" /> Instant Delivery
                  </span>
                )}
                {product.isHot && (
                  <span className="px-2.5 py-1 rounded-md bg-zenvo-accent text-zenvo-bg text-[10px] font-bold uppercase shadow-sm">
                    TRENDING
                  </span>
                )}
                {product.discountPercent && (
                  <span className="px-2.5 py-1 rounded-md bg-zenvo-error text-white text-[10px] font-bold">
              ...
              <button
                ...
                className={`absolute top-3 right-3 p-2 rounded-md border transition-all ${
                  isWishlisted
                    ? 'bg-zenvo-error/20 border-zenvo-error/40 text-zenvo-error'
                    : 'bg-zenvo-bg/70 border-zenvo-border text-zenvo-text-muted hover:text-zenvo-error hover:border-zenvo-error/40'
                }`}
                aria-label="Wishlist"
              >
                <Heart className={`w-4 h-4 ${isWishlisted ? 'fill-zenvo-error' : ''}`} />
              </button>
            </div>

            <div className="flex flex-col justify-between">
              <div>
                <p className="text-[11px] uppercase tracking-widest text-zenvo-text-muted font-semibold mb-2">
                  {product.publisher || product.category.replace('-', ' ')}
                </p>
                <h1 className="text-2xl sm:text-3xl font-black text-zenvo-text tracking-tight leading-tight mb-3">
                  {product.title}
                </h1>
                <div className="flex items-center gap-3 mb-4 text-xs">
                  <div className="flex items-center gap-1 text-zenvo-text-secondary">
                    <Star className="w-3.5 h-3.5 fill-zenvo-accent text-zenvo-accent" />
                    <span className="font-semibold text-zenvo-text">{product.rating}</span>
                    <span className="text-zenvo-text-muted">({product.reviewCount.toLocaleString()} reviews)</span>
                  </div>
                  <span className="w-1 h-1 rounded-full bg-zenvo-border" />
                  <span className="text-zenvo-text-muted capitalize">{product.region}</span>
                </div>
              ...
              <div className="mt-6 grid grid-cols-3 gap-3 text-xs">
                {[
                  { I: Zap, label: product.deliveryType, sub: '≤30 secs' },
                  { I: ShieldCheck, label: 'Verified', sub: 'Official' },
                  { I: Clock, label: '24/7 Live', sub: 'Support' },
                ].map(({ I, label, sub }) => (
                  <div key={label} className="p-3 rounded-xl bg-zenvo-surface/60 border border-zenvo-border text-center">
                    <I className="w-4 h-4 mx-auto mb-1.5 text-zenvo-primary" />
                    <p className="font-bold text-zenvo-text">{label}</p>
                    <p className="text-[10px] text-zenvo-text-muted">{sub}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Tab content: How to find Player ID, reviews */}
          <div className="rounded-2xl bg-zenvo-card border border-zenvo-border overflow-hidden">
            <div className="px-5 sm:px-6 py-4 border-b border-zenvo-border flex gap-5 text-sm font-semibold">
              <div className="text-zenvo-primary border-b-2 border-zenvo-primary pb-3 -mb-4">How to Top-Up</div>
              <div className="text-zenvo-text-secondary hover:text-zenvo-text">Reviews</div>
            </div>
            <div className="p-5 sm:p-6 space-y-4">
              {product.howToFindPlayerId?.map((s, i) => (
                <div key={i} className="flex gap-4">
                  <div className="shrink-0 w-8 h-8 rounded-lg bg-zenvo-primary-soft text-zenvo-primary font-bold flex items-center justify-center text-sm">
                    {i + 1}
                  </div>
                  <p className="text-sm text-zenvo-text-secondary pt-1.5">{s}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="sticky top-24 space-y-5">
            ...
            <div className="rounded-2xl bg-zenvo-card border border-zenvo-border p-5">
              <h3 className="text-sm font-black uppercase tracking-wider text-zenvo-text mb-4 flex items-center gap-2">
                <Gamepad2 className="w-4 h-4 text-zenvo-primary" />
                {product.playerIdLabel}
              </h3>
              <div className="space-y-3">
                <div>
                  <label className="text-[11px] font-bold uppercase tracking-wider text-zenvo-text-muted block mb-1.5">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3.5 py-2.5 rounded-lg bg-zenvo-surface border border-zenvo-border focus:border-zenvo-primary-border focus:ring-2 focus:ring-zenvo-primary-border/40 outline-none transition-all text-sm font-mono"
                    />
                    <button
                      onClick={verifyPlayer}
                      disabled={!playerId.trim() || verifying}
                      className="px-4 py-2.5 rounded-lg bg-zenvo-primary-soft hover:bg-zenvo-primary hover:text-white disabled:opacity-50 text-zenvo-primary text-sm font-bold transition-colors inline-flex items-center gap-1.5 shrink-0"
                    >
                      {verifying ? 'Verifying...' : (
                ...
                {verifiedPlayer && (
                  <div className="p-3 rounded-xl bg-zenvo-success-soft/60 border border-zenvo-success/30 flex items-center gap-3">
                    <CheckCircle2 className="w-5 h-5 text-zenvo-success shrink-0" />
                    <div className="min-w-0">
                      <p className="text-xs text-zenvo-text-muted">Player found:</p>
                ...
                {(product.requiresServerId || product.hasServerId) && (
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2.5 rounded-lg bg-zenvo-surface border border-zenvo-border focus:border-zenvo-primary-border focus:ring-2 focus:ring-zenvo-primary-border/40 outline-none transition-all text-sm"
                    />
                  </div>
                )}
              </div>
            ...
            {/* 2. Denominations */}
            <div className="rounded-2xl bg-zenvo-card border border-zenvo-border p-5">
              <h3 className="text-sm font-black uppercase tracking-wider text-zenvo-text mb-4">Select Package</h3>
                  ...
                  return (
                    <button
                      ...
                      className={`relative text-left p-3 rounded-xl border transition-all active:scale-[0.98] ${
                        active
                          ? 'bg-zenvo-primary-soft/60 border-zenvo-primary-border ring-2 ring-zenvo-primary-border/40 shadow-sm'
                          : 'bg-zenvo-surface/60 border-zenvo-border hover:border-zenvo-border-hover'
                      }`}
                    >
                      {save > 0 && (
                        <span className="absolute -top-1.5 -right-1.5 px-1.5 py-0.5 rounded-md bg-zenvo-error text-white text-[9px] font-bold">
                          -{save}%
                        </span>
                      )}
                      <p className={`font-black font-mono leading-tight ${active ? 'text-zenvo-primary' : 'text-zenvo-text'}`}>
                        {d.label || d.name}
                      </p>
                      <p className="text-xs mt-1 text-zenvo-text-secondary font-bold">
                        {formatCurrency(d.amount, selectedCurrency)}
                      </p>
                      {(d.bonusAmount || d.bonus || d.bonusLabel) && (
                        <p className="text-[10px] mt-0.5 text-zenvo-accent font-semibold">{d.bonusLabel || d.bonus}</p>
                      )}
                    </button>
                  );
                })}
              ...
              <div className="mt-5 flex items-center justify-between">
                <div>
                  <span className="text-[11px] uppercase font-bold tracking-wider text-zenvo-text-muted">Quantity</span>
                </div>
                <div className="flex items-center gap-3 p-1 rounded-lg bg-zenvo-surface border border-zenvo-border">
                  <button
                    onClick={() => setQuantity((q) => Math.max(1, q - 1))}
                    className="p-2 rounded-md text-zenvo-text-secondary hover:bg-zenvo-card hover:text-zenvo-text transition-colors"
                    aria-label="Decrease"
                  >
                    <Minus className="w-4 h-4" />
                  </button>
                  <span className="min-w-[40px] text-center font-mono font-black text-zenvo-text">{quantity}</span>
                  <button
            ...
            <div className="rounded-2xl bg-zenvo-card border border-zenvo-border p-5">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="e.g. ZENVO2026"
                  className="flex-1 px-3.5 py-2.5 rounded-lg bg-zenvo-surface border border-zenvo-border focus:border-zenvo-accent-border focus:ring-2 focus:ring-zenvo-accent-border/40 outline-none transition-all text-sm font-mono uppercase"
                />
                <button
                  onClick={applyCoupon}
                  className="px-4 py-2.5 rounded-lg bg-zenvo-accent hover:bg-zenvo-accent-hover text-zenvo-bg text-sm font-bold transition-colors shrink-0"
                >
                  Apply
                </button>
              </div>
            ...
            <div className="rounded-2xl bg-zenvo-card border border-zenvo-border overflow-hidden">
              <div className="px-5 pt-5">
                <h3 className="text-sm font-black uppercase tracking-wider text-zenvo-text mb-3 flex items-center gap-2">
                  <CreditCard className="w-4 h-4 text-zenvo-primary" /> Payment Method
                </h3>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    { id: 'Zenvo Wallet', I: Wallet, tag: user.walletBalanceUSD > 0 ? `$${user.walletBalanceUSD}` : 'Empty' },
                  ].map(({ id, I: Ic, tag }) => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative p-3 rounded-xl border transition-all flex items-center gap-2.5 text-left ${
                          active
                            ? 'bg-zenvo-primary-soft/50 border-zenvo-primary-border'
                            : 'bg-zenvo-surface/60 border-zenvo-border hover:border-zenvo-border-hover'
                        }`}
                      >
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${active ? 'bg-zenvo-primary/15 text-zenvo-primary' : 'bg-zenvo-card text-zenvo-text-muted'}`}>
                          <Ic className="w-4 h-4" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className={`text-xs font-bold truncate ${active ? 'text-zenvo-primary' : 'text-zenvo-text'}`}>{id}</p>
              ...
              {/* Summary */}
              <div className="px-5 pt-5 mt-2 space-y-2 text-sm">
                ...
                <div className="flex items-baseline justify-between">
                  <span className="text-sm font-bold uppercase tracking-wider text-zenvo-text-muted">Total Amount</span>
              ...
              <div className="p-5 pt-4 space-y-2.5">
                <button
                  ...
                  className="w-full py-3.5 rounded-xl bg-gradient-to-r from-zenvo-primary via-blue-600 to-indigo-600 hover:shadow-primary text-white text-sm font-black uppercase tracking-wider disabled:opacity-50 transition-all active:scale-[0.98] flex items-center justify-center gap-2 shadow-md"
                >
                  {submitting ? (
                ...
                <button
                  ...
                  className="w-full py-3 rounded-xl bg-zenvo-surface border border-zenvo-border hover:border-zenvo-primary-border hover:bg-zenvo-primary-soft/40 disabled:opacity-50 text-sm font-bold text-zenvo-text-secondary hover:text-zenvo-primary transition-all flex items-center justify-center gap-2"
                >
                  <Plus className="w-4 h-4" />
                  Add to Cart
                </button>
                <div className="flex items-center justify-center gap-3 pt-1 text-[10px] font-semibold uppercase tracking-wider text-zenvo-text-muted">
                  <span className="flex items-center gap-1"><ShieldCheck className="w-3.5 h-3.5" /> SSL Secured</span>
                  <span className="flex items-center gap-1"><MessageCircle className="w-3.5 h-3.5" /> 24/7 Support</span>
                  <span className="flex items-center gap-1"><Copy className="w-3.5 h-3.5" /> Refundable</span>
                </div>
              </div>
            </div>
          </div>
